...
public class MainPageManager : MonoBehaviour
{
    ...
    const float TIME_BETWEEN_UPDATES = 100f; // Each 100 seconds update main page data
    const float TRANSITION_SPEED = 0.5f;
    const float SEARCH_SUGGESTION_LIMIT = 10;

    const int SLEEP_POSITION = -70;
    ...
    public GameObject lastNewsCardPrefab;
    ...
    public GameObject NewsLoadingPanel;
    ...
    public Transform CoinInfoTransform;
    ...
    public Texture twitter_logo;
    public Texture rss_logo;
    public Texture yt_logo;
    ...
    void Start()
    {
        ...
        object firstStartup;
        user.info.TryGetValue("firstStartup", out firstStartup);
        if(firstStartup == null || (bool)firstStartup == false) {
            MsgTitle.GetComponent<Text>().text = "Welcome to CoinSentinel";
            MsgContent.GetComponent<Text>().text = "\n\n We are excited to provide you with the tools and information you need to stay on top of the latest developments in the world of cryptocurrencies. \n\n We hope you find our app useful and informative but we must state that this app is not intended to provide financial advice, investment advice, or any other form of advice. \n\n Any actions taken by users based on information provided by this app are done at their own risk. The app is not responsible for any losses or damages incurred as a result of using the app or its information. \n The app and its creators are not affiliated with any cryptocurrency or financial institution. \n\n All information provided by the app is for educational and informational purposes only. Use of the app and its information is subject to the user's own judgement and discretion.";
    ...
    void Update()
    {
        ...
        if(currTime > TIME_BETWEEN_UPDATES) {
            if (currentSection == Sections.HomePage) {
                webClient.ReloadMainMarketsData();
            } else {
    ...
    public void UpdateCoinsData(DataStructures.Coin[] topCoins) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            StartCoroutine(webClient.LoadImageCoroutine(topPerformingCoins[i].id, topPerformingCoins[i].image, smallCoinCard.transform.GetChild(0).gameObject));
            ...
            StartCoroutine(webClient.LoadImageCoroutine(worstPerformingCoins[i].id, worstPerformingCoins[i].image, smallCoinCard2.transform.GetChild(0).gameObject));
    ...
    public void UpdateNewsData(DataStructures.News[] news) {

        var firstLoad = true;

        Debug.Log("NEWS - updating news");

        NewsLoadingPanel.SetActive(false);
        ...
        for (int i = 0; i < news.Length; i++) {
            ...
            if (firstLoad) {
                ...
                GameObject newsCard = Instantiate(newsCardPrefab);
                newsCard.transform.SetParent(svNews.transform);
                SetNewsCardData(newsCard, news[i]);
    ...
    public void SetNewsCardData(GameObject card, DataStructures.News news) {

        card.GetComponent<Button>().onClick.RemoveAllListeners();

        var topTab = card.transform.GetChild(0);
        topTab.GetChild(0).GetComponent<RawImage>().texture = (news.medium == "twitter") ? twitter_logo : rss_logo;
        topTab.GetChild(1).GetComponent<Text>().text = (news.source.StartsWith("@")) ? news.source : "@"+ news.source;
        topTab.GetChild(2).GetComponent<Text>().text = news.pubDate;

        card.transform.GetChild(1).GetComponent<Text>().text = news.title;

        card.GetComponent<Button>().onClick.AddListener(() => Application.OpenURL(news.url));
    ...
    public void SetCoinCardData(GameObject card, DataStructures.Coin coin, int rank) {
        StartCoroutine(webClient.LoadImageCoroutine(coin.id, coin.image, card.transform.GetChild(3).gameObject));
        ...
        cardChild2.GetChild(0).GetComponent<Text>().text = Utils.ReturnFormattedPrice(coin.current_price) + " $";
    ...
    public void UpdateChartData(LineDataSet set, double min, double max, string[] horizontal_values, BarDataSet volumes) {
        ...
        lineChart.SetDirty(); // Refresh chart after data change
    ...
    public void addCardToTrendingCoins(string id, string name, string img_url, double price, double price_change_24, string symbol) {
        ...
        StartCoroutine(webClient.LoadImageCoroutine(id, img_url, smallCoinCard.transform.GetChild(0).gameObject));
        smallCoinCard.transform.GetChild(1).GetComponent<Text>().text = name;
    ...
    public void OnCoinTap(string id, string name, string symbol) {
        ...
        if (webClient.coinDetails.ContainsKey(id)) {
            webClient.coinDetails.TryGetValue(id, out coin); //Coin data must be updated: Price, price change, cap, news...
            StartCoroutine(OnCoinDataLoaded(id, coin.image, CoinInfoTransform.GetChild(0).gameObject, coin));
        } else {
            webClient.RetrieveCoinData(id, true, () => StartCoroutine(OnCoinDataLoaded(id, coin.image, CoinInfoTransform.GetChild(0).gameObject)));
        }

        webClient.RetrieveCoinNews(symbol);

        CoinTitle.transform.GetChild(0).GetComponent<Text>().text = "<color=7BFFEF>" + name + "</color>" + " (" + symbol + ")";

        /* TRANSITIONS */
        MainTitle.transform.DOLocalMoveY(300, TRANSITION_SPEED);
        MenuButton.transform.DOLocalMoveY(300, TRANSITION_SPEED);
        SearchBar.transform.DOLocalMoveY(300, TRANSITION_SPEED);
    ...
    public IEnumerator OnCoinDataLoaded(string id, string imageUrl, GameObject container, DataStructures.CoinDetails coin = null) {
        StartCoroutine(webClient.LoadImageCoroutine(id, imageUrl, container)); //Load Image

        if (coin == null)
            webClient.coinDetails.TryGetValue(id, out coin);
        ...
        var bottomSectionPriceChangeTable = CoinInfoTransform.GetChild(7);

        CoinInfoTransform.GetChild(1).GetComponent<TextMeshProUGUI>().text = Utils.ReturnFormattedPrice(double.Parse(coin.price.ToString()))+ "<size=30>USD</size> ";

        CoinInfoTransform.GetChild(2).GetComponent<Button>().onClick.RemoveAllListeners();
        CoinInfoTransform.GetChild(2).GetComponent<Button>().onClick.AddListener(() => OnFavoriteTapFromCoinInfo(coin.name, coin.id, CoinInfoTransform.GetChild(2)));
        RawImage img = CoinInfoTransform.GetChild(2).GetComponent<RawImage>();
        if (user.favorites.ContainsKey(coin.name)) {
            img.texture = starEnabled;
            img.color = Color.white;
        } else { 
        ...
        CoinInfoTransform.GetChild(5).GetComponent<TextMeshProUGUI>().text = color + Math.Round(coin.price_change_percentage_24h, 2).ToString("F2") + "%</size></color>";
        CoinInfoTransform.GetChild(3).GetComponent<Text>().text = Utils.ReturnFormattedUSNumber(coin.market_cap.ToString()) + " <size=20>USD</size>";
        CoinInfoTransform.GetChild(4).GetComponent<Text>().text = "#"+ coin.market_cap_rank.ToString();

        CoinInfoTransform.GetChild(9).GetComponent<Text>().text = Utils.ReturnFormattedPrice(double.Parse(coin.total_volume.ToString())) + " <size=20>USD</size>";
        CoinInfoTransform.GetChild(10).GetComponent<Text>().text = Utils.ReturnFormattedPrice(double.Parse(coin.high24.ToString())) + " <size=20>USD</size>";
        CoinInfoTransform.GetChild(11).GetComponent<Text>().text = Utils.ReturnFormattedPrice(double.Parse(coin.low24.ToString())) + " <size=20>USD</size>";
    ...
    private void ShowFavoriteCoins() {
        if (!showAll) {
            ...
            webClient.RetrieveCoinsSimplePrice(queryIDs, extraFavCards, names);
        } else {
    ...
    public void OnSearchInput() {
        ...
        if (input.Length != 0) {
            ...
            foreach (DataStructures.Coin o in result.Values) {
                ...
                StartCoroutine(webClient.LoadImageCoroutine(o.id, o.image, searchResult.transform.GetChild(2).gameObject));
    ...
    public void OnSearchTap() {
        ...
        if (TrendingPanel.transform.childCount < 2) {
            for (int i = 0; i < 7; i++) {
                ...
                StartCoroutine(webClient.LoadImageCoroutine(trendingCoins[i].id, trendingCoins[i].image, trendingCoin.transform.GetChild(2).gameObject));
    ...
    public void OnCloseDisclaimerMessage() {
        MessagePanel.SetActive(false);
        user.info["firstStartup"] = true;
        user.SaveUserData();
    }
    ...
    public void OnNewsSelected() {
        JumpToSection(Sections.HiddenPage); // TODO WIP
    }
    ...
    public void JumpToSection(Sections section, bool back = false) {
        ...
        switch (section) {
            case Sections.HomePage:
                ...
                ShowUIElements(false, true, true, false, false, false, false);
            ...
            case Sections.SearchPage:
                ...
                ShowUIElements(true, false, false, false, false, true, false);
                if (!back)
            ...
            case Sections.CoinInfoPage:
                ...
                ShowUIElements(false, false, false, false, true, true, false);
            ...
            case Sections.PortfolioPage:
                ...
                ShowUIElements(false, true, false, true, false, false, false);
                user.backstack.Clear();
                SetActiveButton(1);
                break;
            case Sections.AddCoinPortfolio:
                EnablePanels(false, false, false, false, false, true, true);
                ShowUIElements(false, false, false, true, false, true, false);
                user.backstack.Push(user.previousSection);
                break;
            ...
            case Sections.CoinDescriptionPanel:
                ...
                user.backstack.Push(user.previousSection);
                break;
            case Sections.NewsPage:
                EnablePanels(false, false, false, false, false, false, false, false, false, true);
                ShowUIElements(false, false, false, false, false, true, true);
                user.backstack.Push(user.previousSection);
                SetActiveButton(2);
                break;
            case Sections.HiddenPage:
                EnablePanels(false, false, false, false, false, false, false, false, false, false, true);
                ShowUIElements(false, false, false, false, false, true, true);
                user.backstack.Push(user.previousSection);
                SetActiveButton(2);
                break;
    ...
    public void ShowUIElements(bool searchBar, bool menuButton, bool mainTitle, bool portfolioTitle, bool coinTitle, bool backButton, bool newsTitle) {
        BackButton.transform.DOLocalMoveY(backButton ? SLEEP_POSITION : 150, TRANSITION_SPEED);
        SearchBar.transform.DOLocalMoveY(searchBar ? SLEEP_POSITION : 150, TRANSITION_SPEED);
        MainTitle.transform.DOLocalMoveY(mainTitle ? SLEEP_POSITION : 150, TRANSITION_SPEED);
        MenuButton.transform.DOLocalMoveY(menuButton ? SLEEP_POSITION : 150, TRANSITION_SPEED);
        CoinTitle.transform.DOLocalMoveY(coinTitle ? SLEEP_POSITION : 150, TRANSITION_SPEED);
        PortfolioTitle.transform.DOLocalMoveY(portfolioTitle ? SLEEP_POSITION : 150, TRANSITION_SPEED);
        NewsTitle.transform.DOLocalMoveY(newsTitle ? SLEEP_POSITION : 150, TRANSITION_SPEED);
    }

    public void EnablePanels(bool TopPanelEnabled, bool MarketsScrollViewEnabled, bool TrendingPanelEnabled, bool SearchPanelEnabled, bool CoinInfoPanelEnabled, bool PortfolioPanelEnabled, bool AddCoinWindowEnabled = false, bool PortfolioCoinMenu = false, bool CoinDescription = false, bool NewsPanelEnabled = false, bool HiddenPageEnabled = false) {
        ...
        NewsPanel.SetActive(NewsPanelEnabled);
        HiddenPage.SetActive(HiddenPageEnabled);
    }
