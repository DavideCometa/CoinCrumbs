...
public class PortfolioPageManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI numCoinsValueTxt;
    ...
    void Start() {
        ...
        numCoinsValueTxt.text = userData.portfolio.Count.ToString();
    ...
    public void UpdatePortfolioCoinPrice() {
        foreach(Transform coinCard in portfolioSv.transform) {
            ...
            StartCoroutine(webClient.UpdateCoinPriceForPortfolio(pData.id, "https://api.coingecko.com/api/v3/simple/price?ids=" + pData.id + "&vs_currencies=usd&include_24hr_change=true&include_last_updated_at=true", fields, pData.amount, pData.symbol));
        }
    ...
    public void SetPortfolioCardData(GameObject portfolioCard, Dictionary<string, object> c = null, PortfolioCoinData pData = null) {
        ...
        StartCoroutine(webClient.LoadImageCoroutine(pData.id, null, portfolioCard.transform.GetChild(2).gameObject));
    }
